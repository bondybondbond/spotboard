...
- **Excluding a section's heading could reappear after refresh** (#71): the excluded heading's own text was being stored as the card's "refresh anchor," so a later self-healing refresh could rebuild the section around exactly the heading you asked to hide, and the exclusion silently stopped applying. Excluded headings (and the auto-generated card name, when it independently matched one) are no longer used as that anchor, so an excluded heading now stays excluded across refreshes (`src/content.ts`).
