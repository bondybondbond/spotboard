...
- **Excluding items on endlessly-scrolling pages no longer blanks the preview** (#112): on sites that only keep the posts near your screen (e.g. Peerlist Scroll), scrolling down while excluding used to leave the preview blank and quietly forget the exclusions you'd already made. The preview now always shows what's left, and if an excluded item scrolled out of the page you get a short note saying it wasn't kept (so you can exclude it again). An exclusion is only re-applied to the same content coming back, never to a different post that took its place. Capturing every post on such pages is still a separate limitation (#111) (`src/content.ts`).
