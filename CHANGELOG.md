...
- **Captured lists no longer get bullets or doubled numbers from SpotBoard** (#102): bullet lists show no added bullets, and a numbered list only gets SpotBoard's numbers when the site's own numbers aren't in the captured content — so BBC-style "1. 1 headline" double numbering is gone while ranked lists whose numbers were only drawn by the site's styling (Guardian, Verge) keep their ranking. The capture preview and the dashboard card look the same (`src/utils/dom-cleanup.ts`, `src/content.ts`).
